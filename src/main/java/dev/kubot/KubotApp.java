...
public class KubotApp extends Application {
    ...
    private void updatePanelVisibility() {
        ...
        Platform.runLater(this::setPanelDividers);
    }
    ...
    private VBox buildPodPane() {
        ...
        TableColumn<V1Pod, String> owner = column("Managed By", this::managedByLabel, 220);
    ...
    private TabPane buildDetailsPane() {
        ...
        VBox logsPane = new VBox(8, logsBar, logsText);
    ...
    private void refreshNamespaces() {
        run("Loading namespaces", kubernetes::namespaces, loaded -> {
            namespaces.setAll(loaded.stream()
                    .sorted(Comparator
                            .comparing((V1Namespace ns) -> isSystemNamespace(name(ns.getMetadata())))
                            .thenComparing(ns -> name(ns.getMetadata())))
                    .toList());
    ...
    private void loadPods(String namespace) {
        run("Loading pods in " + namespace, () -> kubernetes.pods(namespace), loaded -> {
            pods.setAll(loaded.stream()
                    .sorted(Comparator.comparing(pod -> name(pod.getMetadata())))
                    .toList());
            if (!pods.isEmpty()) {
    ...
    private void loadEvents(String namespace, V1Pod pod) {
        ...
        task.setOnSucceeded(done -> {
            ...
            List<CoreV1Event> matching = loaded.stream()
                    .filter(podEvent -> {
    ...
    private void loadLogs(int lines) {
        ...
        run("Loading logs", () -> kubernetes.logs(selectedNamespace, podName, container, requestedLogLines, null), logs -> {
            logsText.setText(logs == null || logs.isBlank() ? "(no logs returned)" : logs);
    ...
    private String managedByLabel(V1OwnerReference owner) {
        ...
        if ("ReplicaSet".equals(owner.getKind())) {
            return deploymentForReplicaSet(owner.getName())
                    .map(deployment -> "Deployment " + deployment)
                    .orElse("ReplicaSet " + simplifyReplicaSetName(owner.getName()));
        }
        return owner.getKind() + " " + owner.getName();
    }

    private java.util.Optional<String> deploymentForReplicaSet(String replicaSetName) {
        return currentLookup.replicaSets().stream()
                .filter(replicaSet -> replicaSet.getMetadata() != null && Objects.equals(replicaSet.getMetadata().getName(), replicaSetName))
                .flatMap(replicaSet -> replicaSet.getMetadata().getOwnerReferences() == null
                        ? java.util.stream.Stream.empty()
                        : replicaSet.getMetadata().getOwnerReferences().stream())
                .filter(owner -> "Deployment".equals(owner.getKind()) && owner.getName() != null)
    ...
    private java.util.Optional<String> guessDeploymentName(String replicaSetName) {
        ...
        return currentLookup.deployments().stream()
                .map(V1Deployment::getMetadata)
                .filter(Objects::nonNull)
                .map(V1ObjectMeta::getName)
                .filter(Objects::nonNull)
                .filter(name -> replicaSetName.startsWith(name + "-"))
