...
public class RelationResolver {
    public List<RelatedResource> resolve(V1Pod pod, ResourceLookup lookup) {
        ...
        return related.stream()
                .sorted(Comparator.comparingInt(RelatedResource::sortPriority)
                        .thenComparing(RelatedResource::kind)
                        .thenComparing(RelatedResource::name))
                .toList();
    }
    ...
    public String decodedSecretData(V1Secret secret) {
        ...
        return secret.getData().entrySet().stream()
                ...
                .map(entry -> entry.getKey() + " = " + decode(entry.getValue()))
    ...
    private void addOwners(V1Pod pod, ResourceLookup lookup, List<RelatedResource> related) {
        ...
        for (V1OwnerReference owner : owners) {
            ...
            if ("ReplicaSet".equals(owner.getKind())) {
                findReplicaSet(owner.getName(), lookup)
                        .flatMap(rs -> ownerReferences(rs.getMetadata()).stream()
                                .filter(ref -> "Deployment".equals(ref.getKind()))
                                .findFirst())
                        .ifPresent(deploymentOwner -> addOwner(deploymentOwner, lookup, related));
            }
        }
    }
    ...
    private void addOwner(V1OwnerReference owner, ResourceLookup lookup, List<RelatedResource> related) {
        ...
        related.add(new RelatedResource(
                "owner",
                owner.getKind(),
                owner.getName(),
                resource == null ? "Owner reference found; resource was not available." : "Pod is owned by this workload.",
                resource == null ? "" : YamlSupport.dump(resource)
        ));
    }
            ...
            for (V1Volume volume : spec.getVolumes()) {
                ...
                if (secret != null && secret.getSecretName() != null) {
                    ...
                    if (secret.getItems() != null && !secret.getItems().isEmpty()) {
                        detail += "; keys: " + secret.getItems().stream().map(V1KeyToPath::getKey).filter(Objects::nonNull).collect(Collectors.joining(", "));
                    }
        ...
        if (container.getEnv() != null) {
            for (V1EnvVar env : container.getEnv()) {
                ...
                if (source.getConfigMapKeyRef() != null && source.getConfigMapKeyRef().getName() != null) {
                    addConfigMap("env", source.getConfigMapKeyRef().getName(),
                            containerName + " env " + env.getName() + " from key " + source.getConfigMapKeyRef().getKey(),
                            lookup, related);
                }
                if (source.getSecretKeyRef() != null && source.getSecretKeyRef().getName() != null) {
                    addSecret("env", source.getSecretKeyRef().getName(),
                            containerName + " env " + env.getName() + " from key " + source.getSecretKeyRef().getKey(),
                            lookup, related);
                }
            }
        }
        if (container.getEnvFrom() != null) {
            for (V1EnvFromSource envFrom : container.getEnvFrom()) {
                if (envFrom.getConfigMapRef() != null && envFrom.getConfigMapRef().getName() != null) {
                    addConfigMap("envFrom", envFrom.getConfigMapRef().getName(),
                            containerName + " imports all keys", lookup, related);
                }
                if (envFrom.getSecretRef() != null && envFrom.getSecretRef().getName() != null) {
                    addSecret("envFrom", envFrom.getSecretRef().getName(),
                            containerName + " imports all keys", lookup, related);
                }
            }
        }
    ...
    private void addMatchingServices(V1Pod pod, ResourceLookup lookup, List<RelatedResource> related) {
        ...
        for (V1Service service : lookup.services()) {
            ...
            if (selector != null && !selector.isEmpty() && labels.entrySet().containsAll(selector.entrySet())) {
                related.add(new RelatedResource("service selector", "Service", name(service.getMetadata()),
                        "Service selector matches this pod's labels: " + selector, YamlSupport.dump(service)));
            }
        }
    }
    ...
    private void addConfigMap(String source, String name, String detail, ResourceLookup lookup, List<RelatedResource> related) {
        ...
        related.add(new RelatedResource(source, "ConfigMap", name,
                configMap == null ? detail + "; ConfigMap was not available." : detail,
                configMap == null ? "" : YamlSupport.dump(configMap)));
    }
    ...
    private void addSecret(String source, String name, String detail, ResourceLookup lookup, List<RelatedResource> related) {
        ...
        related.add(new RelatedResource(source, "Secret", name,
                secret == null ? detail + "; Secret was not available." : detail,
                secret == null ? "" : YamlSupport.dump(secret) + decoded));
    }
